...
- `ecommerce-template` is the tenant runtime/template. Do not infer complete self-service onboarding, admin-plane ownership or provisioning completeness from a green storefront runtime.
- A healthy `campifruit`-style tenant proves runtime health only. It is not evidence that the full control plane is complete.
