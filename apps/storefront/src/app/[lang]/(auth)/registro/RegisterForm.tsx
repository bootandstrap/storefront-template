...
const ERROR_MESSAGES: Record<string, string> = {
    fields_required: 'auth.fieldsRequired',
    ...
    return (
        <>
            <form action={formAction} className="space-y-4">
                ...
                <div>
                    <label htmlFor="full_name" className="block text-sm font-medium text-tx-sec mb-1">
                        {t('auth.fullName')}
                    </label>
                    <input
                        id="full_name"
                ...
                <div>
                    <label htmlFor="email" className="block text-sm font-medium text-tx-sec mb-1">
                        {t('auth.email')}
                    </label>
                    <input
                        id="email"
                ...
                <div>
                    <label htmlFor="password" className="block text-sm font-medium text-tx-sec mb-1">
                        {t('auth.password')}
                    </label>
                    <input
                        id="password"
            ...
            {showGoogleAuth && (
                <>
                    <div className="relative">
                        <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-sf-3" /></div>
                        <div className="relative flex justify-center text-xs">
                            <span className="px-2 bg-sf-0 text-tx-muted">{t('auth.orContinueWith')}</span>
                        </div>
                    </div>
                    <a
                        href={`/${lang}/auth/callback?provider=google`}
                        className="btn btn-ghost w-full border border-sf-3 text-sm"
                    >
            ...
            <p className="text-center text-sm text-tx-muted">
                {t('auth.hasAccount')}{' '}
                <Link href={`/${lang}/login`} className="text-brand font-medium hover:underline">
                    {t('auth.login')}
                </Link>
            </p>
        </>
