...
export function createModuleRegistry(contract) {
    ...
    for (const mod of contract.modules?.catalog ?? []) {
        ...
        const tiers = (mod.tiers ?? []).map((tier, index) => {
            ...
            for (const f of Object.keys(flagEffects))
                allFlags.add(f);
            for (const l of Object.keys(limitEffects))
                allLimits.add(l);
            return {
                level: index + 1,
                name: tier.name,
                pricing: {
                    CHF: { amount: tier.price_chf ?? 0, interval: 'month' },
                },
    ...
    registry.getAllModules = () => Object.values(modules);
    registry.getFeatureGateMap = () => getFeatureGateMap(registry);
    registry.getPanelPolicy = () => getPanelPolicy(registry);
    return registry;
}
 ...
 */
export function getFeatureGateMap(registry) {
    const mappings = [];
 ...
 */
export function getPanelPolicy(registry) {
    const policies = [];
...
/** Get a module by key */
export function getModule(registry, key) {
...
export function findModuleByFlag(registry, flag) {
