...
export default withGovernanceGate("enable_ecommerce", async ({
    ...
    try {
        ...
        // Always dispatches — channels self-gate based on config availability
        await dispatchToChannels("order.placed", eventPayload)
        ...
        await notifyStorefront("order.placed", eventPayload, "order-placed")
