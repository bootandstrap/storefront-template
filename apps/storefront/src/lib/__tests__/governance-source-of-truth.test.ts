...
import { FALLBACK_CONFIG as SharedFALLBACK_CONFIG } from "../../../../../packages/shared/src/governance/defaults";
...
describe("Governance Source of Truth — E2E Drift Detection", () => {
  ...
  describe("flags: contract ↔ consumers", () => {
    ...
    it("contract flags = SharedFALLBACK_CONFIG.featureFlags keys (package)", () => {
      const sharedDefaultKeys = sorted(
        Object.keys(SharedFALLBACK_CONFIG.featureFlags),
      );
      ...
      for (const flag of contract.flags.keys) {
        ...
        expect(
          FALLBACK_CONFIG.featureFlags[
            flag as keyof typeof FALLBACK_CONFIG.featureFlags
          ],
  ...
  describe("limits: contract ↔ consumers", () => {
    ...
    it("contract limits = SharedFALLBACK_CONFIG.planLimits keys (package)", () => {
      const sharedDefaultKeys = sorted(
        Object.keys(SharedFALLBACK_CONFIG.planLimits),
      );
    ...
    it("numeric_keys + metadata_keys = all keys", () => {
      const combined = sorted([
        ...contract.limits.numeric_keys,
        ...contract.limits.metadata_keys,
      ]);
      ...
      for (const key of contract.limits.numeric_keys) {
        const val =
          FALLBACK_CONFIG.planLimits[
            key as keyof typeof FALLBACK_CONFIG.planLimits
          ];
  ...
  describe("modules: contract ↔ consumers", () => {
    ...
    it("every contract module has at least one flag in FEATURE_GATE_MAP", () => {
      const gatedModules = new Set(
        Object.values(FEATURE_GATE_MAP).map((e) => e.moduleKey),
      );
    ...
    it("contract catalog entries match contract module keys", () => {
      const catalogKeys = sorted(
        contract.modules.catalog.map((m: { key: string }) => m.key),
      );
  ...
  describe("pricing integrity", () => {
    ...
    it("every module tier has a unique key within its module", () => {
      for (const mod of contract.modules.catalog) {
        ...
        expect(unique.size, `Module ${mod.key} has duplicate tier keys`).toBe(
          tierKeys.length,
        );
      }
    });
  ...
  describe("cross-package equality", () => {
    it("inline featureFlags default values = shared package values", () => {
      expect(FALLBACK_CONFIG.featureFlags).toEqual(
        SharedFALLBACK_CONFIG.featureFlags,
      );
    });

    it("inline planLimits default values = shared package values", () => {
      expect(FALLBACK_CONFIG.planLimits).toEqual(
        SharedFALLBACK_CONFIG.planLimits,
      );
    });
  });
  ...
  describe("panel-policy integration", () => {
    it("ROUTE_REDIRECT_MAP module routes reference valid contract modules", () => {
      ...
      const moduleRoutes = Object.entries(ROUTE_REDIRECT_MAP).filter(
        ([, v]) => v.section === "modulos" && v.tab,
      );
      ...
      for (const [key] of moduleRoutes) {
        ...
        expect(
          contractModuleKeys.has(key) ||
            ...
            key === "canales" || // sales_channels module
            key === "capacidad" || // capacity/traffic module
            key === "auth", // auth_advanced module
