...
export const CURRENCY_INFO_MAP: Record<string, CurrencyInfo> = Object.fromEntries(
    CURRENCIES.map(c => [c.code, c])
);

/** code → Intl locale */
export const CURRENCY_INTL_MAP: Record<string, string> = Object.fromEntries(
    CURRENCIES.filter(c => c.intlLocale).map(c => [c.code, c.intlLocale!])
);
