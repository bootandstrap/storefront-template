 ...
 */
export function getModuleActivationUrl(
  flagOrModuleKey: string,
  lang = "es",
): string {
  const entry = FEATURE_GATE_MAP[flagOrModuleKey];
  const moduleKey = entry?.moduleKey ?? flagOrModuleKey;
  if (!moduleKey || !MODULE_SLUGS[moduleKey])
    return `/${lang}/panel/ajustes?tab=suscripcion`;
 ...
 */
export function getFeatureGateEntry(
  flagKey: string,
): FeatureGateEntry | undefined {
