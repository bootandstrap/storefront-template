...
describe("vendor-neutral evidence event v2", () => {
    ...
    ] as Array<Partial<EvidenceEventInput>>)("rejects secrets and raw bodies: %o", (unsafe) => {
    ...
    it("queries an ordered in-memory chain by trace or operation", async () => {
        ...
        await emitEvidence({ ...input, event_name: "event.second" }, sink,
            () => "2026-08-06T20:00:02.000Z", () => "event-2")
        await emitEvidence({ ...input, event_name: "event.first" }, sink,
            () => "2026-08-06T20:00:01.000Z", () => "event-1")
