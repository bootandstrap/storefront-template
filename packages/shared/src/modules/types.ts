...
export type ModuleCategory =
    | 'commerce'      // ecommerce, pos, pos_kiosk
    | 'marketing'     // seo, rrss, email_marketing, automation
    | 'communication' // chatbot, sales_channels
    | 'management'    // crm, capacidad
    | 'platform'      // i18n, auth_advanced
...
export interface ModuleTierDefinition {
    ...
    pricing: Record<string, { amount: number; interval: 'month' | 'year' }>
...
// ── Medusa Integration (future — all modules will use Medusa) ─────────────
...
export interface MedusaModuleIntegration {
    ...
    links?: Array<{
        from: string  // e.g., 'product'
        to: string    // e.g., 'vendor'
...
// ── Feature Gate Map (derived from registry) ──────────────────────────────
...
// ── Panel Policy (derived from registry) ──────────────────────────────────
