...
export function createBillingGateway(options = {}) {
    ...
    switch (provider) {
        case 'stripe': {
            if (!options.store) {
                throw new Error('[BillingGatewayFactory] StripeBillingGateway requires a TenantBillingStore. '
            ...
            const { StripeBillingGateway } = require('./providers/stripe');
            return new StripeBillingGateway(options.store);
        }
        case 'mock':
            return new MockBillingGateway();
        case 'lemon_squeezy':
            throw new Error('[BillingGatewayFactory] LemonSqueezy provider not yet implemented. '
                + 'Planned for future phase. Use Stripe or Mock.');
        case 'paddle':
            throw new Error('[BillingGatewayFactory] Paddle provider not yet implemented. '
                + 'Planned for future phase. Use Stripe or Mock.');
        default:
...
function detectProvider() {
    ...
    if (process.env.NODE_ENV === 'development' || process.env.DEMO_MODE === 'true') {
        console.warn('[BillingGatewayFactory] No STRIPE_SECRET_KEY found — using MockBillingGateway. '
    ...
    console.error('[BillingGatewayFactory] ⚠️ Production environment without STRIPE_SECRET_KEY. '
