 ...
 */
export async function getOwnerModuleStatus(
  locale: string,
...
async function fetchModuleCatalog(
  ...
  if (!rpcError && rpcData && Array.isArray(rpcData) && rpcData.length > 0) {
    ...
    if (!error && modules) {
      dbCatalog = modules.map((m) => ({
        ...
        payment_type: (["one_time", "extension"].includes(m.payment_type || "")
          ? m.payment_type
  ...
  try {
    const contract = await import("@/lib/governance-contract.json");
    ...
    for (const cm of contractCatalog) {
      if (!existingKeys.has(cm.key)) {
        dbCatalog.push({
          ...
          color_gradient:
            CONTRACT_GRADIENT_MAP[cm.key] || "from-gray-500 to-gray-600",
...
async function fetchActiveModules(
  ...
  try {
    ...
    if (flagRow) {
      ...
      const { deriveActiveModulesFromFlags } =
        await import("@/lib/governance/derive-modules");
      const flagModules = deriveActiveModulesFromFlags(
        flagRow as Record<string, boolean | null | undefined>,
      );
  ...
  if (orders) {
    for (const order of orders as Array<{
      ...
      status: string;
      module_order_items:
        | Array<{ module_key: string; tier_name: string | null }>
        | { module_key: string; tier_name: string | null }
        | null;
    }>) {
...
function mapRpcToOwnerModule(row: Record<string, unknown>): OwnerModuleInfo {
  return {
    ...
    color_gradient:
      (row.color_gradient as string) || "from-gray-500 to-gray-600",
    ...
    payment_type: (["one_time", "extension"].includes(
